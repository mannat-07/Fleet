...
class _InsuranceScreenState extends State<InsuranceScreen>
  ...
  Future<void> _loadInsurance() async {
    ...
    try {
      ...
      AppStore.insurance = trucks
          .where((t) => t['insuranceStatus'] != null || t['insuranceExpiry'] != null)
  ...
  Widget build(BuildContext context) {
    ...
    final expiring = AppStore.insurance.where((i) => i.status == 'Expiring').length;
    final expired  = AppStore.insurance.where((i) => i.status == 'Expired').length;
  ...
  Widget _buildBody(FleetColors c) {
    ...
    if (_error != null) return _ErrorState(error: _error!, onRetry: _loadInsurance, c: c);
    if (AppStore.insurance.isEmpty) return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.shield_outlined, color: c.textSub, size: 56),
        const SizedBox(height: 16),
        Text('No insurance records found', style: TextStyle(color: c.textSub, fontSize: 15)),
        const SizedBox(height: 8),
        Text('Add insurance fields to your trucks', style: TextStyle(color: c.textSub, fontSize: 13)),
      ]),
    );

    return RefreshIndicator(
      color: AppColors.orangeStart, backgroundColor: c.surface, onRefresh: _loadInsurance,
...
class _Chip extends StatelessWidget {
  final String label; final int count; final Color color; final FleetColors c;
...
class _InsuranceCard extends StatelessWidget {
  final InsuranceModel doc; final FleetColors c;
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(doc.truckPlate, style: TextStyle(color: c.text, fontSize: 16, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 3),
                  if (doc.provider.isNotEmpty) Text(doc.provider, style: TextStyle(color: c.textSub, fontSize: 13)),
...
class _ErrorState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('Could not load insurance', style: TextStyle(color: c.text, fontSize: 16, fontWeight: FontWeight.w700)),
        const SizedBox(height: 8),
