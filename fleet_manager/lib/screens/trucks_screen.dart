...
class _TrucksScreenState extends State<TrucksScreen>
  ...
  void _confirmDelete(TruckModel truck) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text('Delete Truck', style: TextStyle(color: c.text, fontWeight: FontWeight.w800)),
        content: Text('Remove ${truck.plate} from your fleet?', style: TextStyle(color: c.textSub)),
        actions: [
        ...
        child: SafeArea(
          child: Column(
            children: [
              ScreenHeader(
                title: 'Your Trucks',
                subtitle: _loading ? 'Loading…' : '${AppStore.trucks.length} registered',
  ...
  Widget _buildBody(FleetColors c) {
    ...
    if (_error != null) return _ErrorState(error: _error!, onRetry: _loadTrucks, c: c);
    if (AppStore.trucks.isEmpty) return _EmptyState(
        icon: Icons.local_shipping_outlined,
        message: 'No trucks yet.\nTap + to add your first truck.', c: c);
...
class _TruckCard extends StatelessWidget {
  ...
  const _TruckCard({required this.truck, required this.c, required this.onEdit, required this.onDelete});
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(truck.plate, style: TextStyle(color: c.text, fontSize: 15, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 2),
                  Text('${truck.model}  •  ${truck.type}', style: TextStyle(color: c.textSub, fontSize: 12)),
                  if (truck.location.isNotEmpty) ...[
                    const SizedBox(height: 4),
                    Row(children: [
                      Icon(Icons.location_on_outlined, color: c.textSub, size: 12),
                      const SizedBox(width: 3),
                      Expanded(child: Text(truck.location,
                          style: TextStyle(color: c.textSub, fontSize: 11), overflow: TextOverflow.ellipsis)),
                    ]),
                  ],
                ],
              ),
...
class _TruckFormSheetState extends State<_TruckFormSheet> {
  ...
  @override
  void dispose() { _plateCtrl.dispose(); _modelCtrl.dispose(); _yearCtrl.dispose(); super.dispose(); }

  Future<void> _submit() async {
    final plate = _plateCtrl.text.trim();
    final model = _modelCtrl.text.trim();
    if (plate.isEmpty || model.isEmpty) {
      setState(() => _error = 'Plate and model are required.'); return;
    }
    setState(() { _loading = true; _error = null; });
    try {
      final body = {
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            GlassInput(hint: 'e.g. MH12 AB 1234', icon: Icons.pin, controller: _plateCtrl),
            const SizedBox(height: 14),
            _Label('Truck Model *', c),
            GlassInput(hint: 'e.g. Tata Prima 4928.S', icon: Icons.directions_car, controller: _modelCtrl),
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _Label('Type', c),
                _DropdownField(value: _type, items: _types, c: c, onChanged: (v) => setState(() => _type = v!)),
              ])),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _Label('Year', c),
                GlassInput(hint: '${DateTime.now().year}', icon: Icons.calendar_today_outlined,
                    controller: _yearCtrl, keyboardType: TextInputType.number),
              ])),
            ]),
            const SizedBox(height: 14),
            _Label('Status', c),
            _DropdownField(value: _status, items: _statuses, c: c, onChanged: (v) => setState(() => _status = v!)),
            const SizedBox(height: 28),
            CustomButton(label: _isEdit ? 'Save Changes' : 'Add Truck', onPressed: _submit, isLoading: _loading),
            const SizedBox(height: 8),
          ],
        ),
      ),
...
class _DropdownField extends StatelessWidget {
  final String value; final List<String> items; final FleetColors c;
  final void Function(String?) onChanged;
  const _DropdownField({required this.value, required this.items, required this.c, required this.onChanged});
  @override
  Widget build(BuildContext context) => Container(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
      decoration: BoxDecoration(color: c.inputBg, borderRadius: BorderRadius.circular(16),
          border: Border.all(color: c.inputBorder, width: 1.5)),
      child: DropdownButtonHideUnderline(child: DropdownButton<String>(
        value: value, isExpanded: true, dropdownColor: c.sheetBg,
        style: TextStyle(color: c.text, fontSize: 14),
        icon: Icon(Icons.keyboard_arrow_down, color: c.textSub),
        items: items.map((e) => DropdownMenuItem(value: e, child: Text(e, style: TextStyle(color: c.text)))).toList(),
        onChanged: onChanged,
      )));
}
...
class _ErrorState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('Could not load trucks', style: TextStyle(color: c.text, fontSize: 16, fontWeight: FontWeight.w700)),
        const SizedBox(height: 8),
