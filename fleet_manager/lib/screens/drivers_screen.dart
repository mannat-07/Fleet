...
class _DriversScreenState extends State<DriversScreen>
  ...
  void _openAddSheet() {
    showModalBottomSheet(
      ...
      builder: (_) => _DriverFormSheet(
        ...
        onCredentials: (email, password) => _showCredentialsDialog(email, password),
      ),
    );
  }
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(children: [
          Container(
            ...
            child: const Icon(Icons.check_circle, color: AppColors.green, size: 20),
          ),
          const SizedBox(width: 12),
          Text('Driver Account Created', style: TextStyle(color: c.text, fontSize: 16, fontWeight: FontWeight.w800)),
        ]),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Share these login credentials with the driver:',
            ...
            Container(
              ...
              child: Row(children: [
                ...
                Expanded(child: Text('Save this password — it won\'t be shown again.',
  ...
  void _openEditSheet(DriverModel driver) {
    showModalBottomSheet(
      context: context, isScrollControlled: true, backgroundColor: Colors.transparent,
      builder: (_) => _DriverFormSheet(existing: driver, onSaved: (_) => _loadDrivers()),
    );
  }
  ...
  void _confirmDelete(DriverModel driver) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text('Remove Driver', style: TextStyle(color: c.text, fontWeight: FontWeight.w800)),
        content: Text('Remove ${driver.name} from your team?', style: TextStyle(color: c.textSub)),
        actions: [
        ...
        child: SafeArea(
          child: Column(
            children: [
              ScreenHeader(
                title: 'Drivers',
                subtitle: _loading ? 'Loading…' : '${AppStore.drivers.length} registered',
                actions: [HeaderIconBtn(icon: Icons.person_add_alt_1, onTap: _openAddSheet)],
              ),
  ...
  Widget _buildBody(FleetColors c) {
    if (_loading) return const Center(child: CircularProgressIndicator(color: AppColors.orangeStart));
    if (_error != null) return _ErrorState(error: _error!, onRetry: _loadDrivers, c: c);
    if (AppStore.drivers.isEmpty) return _EmptyState(
        icon: Icons.people_outline, message: 'No drivers yet.\nTap + to add your first driver.', c: c);

    return RefreshIndicator(
      color: AppColors.orangeStart, backgroundColor: c.surface, onRefresh: _loadDrivers,
      child: ListView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          return AnimatedBuilder(
            animation: anim,
            builder: (_, child) => Opacity(opacity: anim.value,
                child: Transform.translate(offset: Offset(0, 20 * (1 - anim.value)), child: child)),
            child: _DriverCard(
              driver: AppStore.drivers[i], c: c,
...
class _DriverCard extends StatelessWidget {
  final DriverModel driver; final FleetColors c;
  final VoidCallback onEdit, onDelete;
  const _DriverCard({required this.driver, required this.c, required this.onEdit, required this.onDelete});
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20), border: Border.all(color: c.cardBorder),
        ),
        child: Row(
          children: [
            Container(
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(driver.name, style: TextStyle(color: c.text, fontSize: 15, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 3),
                  if (driver.phone.isNotEmpty) Row(children: [
                    Icon(Icons.phone_outlined, color: c.textSub, size: 12), const SizedBox(width: 4),
                    Text(driver.phone, style: TextStyle(color: c.textSub, fontSize: 12)),
                  ]),
                  const SizedBox(height: 3),
                  if (driver.assignedTruck.isNotEmpty) Row(children: [
                    Icon(Icons.local_shipping_outlined, color: c.textSub, size: 12), const SizedBox(width: 4),
                    Expanded(child: Text(driver.assignedTruck,
                        style: TextStyle(color: c.textSub, fontSize: 12), overflow: TextOverflow.ellipsis)),
                  ]),
                ],
              ),
            ),
              ...
              children: [
                ...
                Row(children: [
                  GestureDetector(onTap: onEdit,
                      child: Icon(Icons.edit_outlined, color: AppColors.blue.withOpacity(0.8), size: 18)),
                  const SizedBox(width: 10),
                  GestureDetector(onTap: onDelete,
                      child: Icon(Icons.delete_outline, color: AppColors.red.withOpacity(0.7), size: 18)),
                ]),
              ],
            ),
          ],
...
class _DriverFormSheet extends StatefulWidget {
  ...
  const _DriverFormSheet({this.existing, required this.onSaved, this.onCredentials});
...
class _DriverFormSheetState extends State<_DriverFormSheet> {
  late final TextEditingController _nameCtrl, _phoneCtrl, _licCtrl, _emailCtrl, _passCtrl;
  bool _loading = false;
  String? _error;
  ...
  void dispose() {
    ...
    _licCtrl.dispose();  _emailCtrl.dispose(); _passCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _submit() async {
    ...
    if (name.isEmpty || email.isEmpty) {
      setState(() => _error = 'Name and email are required.'); return;
    }
    if (!email.contains('@')) {
    ...
    try {
      final body = {
        ...
        'email': email,
        'phone': phone,
        if (_licCtrl.text.trim().isNotEmpty) 'licenseNumber': _licCtrl.text.trim(),
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            GlassInput(hint: 'driver@example.com', icon: Icons.email_outlined,
                controller: _emailCtrl, keyboardType: TextInputType.emailAddress),
            const SizedBox(height: 14),
            _Label('Password (optional — auto-generated if blank)', c),
            GlassInput(hint: 'Leave blank to auto-generate', icon: Icons.lock_outline,
                controller: _passCtrl, obscure: true),
            const SizedBox(height: 14),
            _Label('Phone Number', c),
            GlassInput(hint: '+91 XXXXX XXXXX', icon: Icons.phone_outlined,
                controller: _phoneCtrl, keyboardType: TextInputType.phone),
            const SizedBox(height: 14),
            _Label('License Number', c),
            GlassInput(hint: 'License number', icon: Icons.badge_outlined, controller: _licCtrl),
            const SizedBox(height: 28),
            CustomButton(label: _isEdit ? 'Save Changes' : 'Add Driver & Create Account',
...
class _ErrorBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
      ...
      decoration: BoxDecoration(color: AppColors.red.withOpacity(0.1),
          borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.red.withOpacity(0.3))),
      child: Row(children: [
        const Icon(Icons.error_outline, color: AppColors.red, size: 16), const SizedBox(width: 8),
...
class _EmptyState extends StatelessWidget {
  final IconData icon; final String message; final FleetColors c;
  const _EmptyState({required this.icon, required this.message, required this.c});
  @override
  Widget build(BuildContext context) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    Icon(icon, color: c.textSub, size: 56), const SizedBox(height: 16),
    Text(message, textAlign: TextAlign.center, style: TextStyle(color: c.textSub, fontSize: 15, height: 1.6)),
  ]));
}

class _ErrorState extends StatelessWidget {
  final String error; final VoidCallback onRetry; final FleetColors c;
  const _ErrorState({required this.error, required this.onRetry, required this.c});
  @override
  Widget build(BuildContext context) => Center(child: Padding(
      padding: const EdgeInsets.all(32),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.cloud_off_outlined, color: c.textSub, size: 48), const SizedBox(height: 16),
        Text('Could not load drivers', style: TextStyle(color: c.text, fontSize: 16, fontWeight: FontWeight.w700)),
        const SizedBox(height: 8),
        Text(error, style: TextStyle(color: c.textSub, fontSize: 13), textAlign: TextAlign.center),
...
class _CredentialRow extends StatefulWidget {
  ...
  final FleetColors c;
  const _CredentialRow({required this.label, required this.value, required this.c});
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(widget.label, style: TextStyle(color: widget.c.textSub, fontSize: 11)),
            const SizedBox(height: 2),
            SelectableText(widget.value,
                style: TextStyle(color: widget.c.text, fontSize: 14, fontWeight: FontWeight.w700)),
          ])),
          GestureDetector(
            onTap: () async {
