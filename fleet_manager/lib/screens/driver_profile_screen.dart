...
class DriverProfileScreen extends StatefulWidget {
  ...
  const DriverProfileScreen({super.key, this.driverProfile, this.assignedTruck});
...
class _DriverProfileScreenState extends State<DriverProfileScreen>
  ...
  void initState() {
    ...
    _nameCtrl  = TextEditingController(text: widget.driverProfile?.name  ?? p.name);
    _phoneCtrl = TextEditingController(text: widget.driverProfile?.phone ?? p.phone);
  }
          ...
          child: FadeTransition(
            ...
            child: CustomScrollView(
              slivers: [
                ...
                SliverToBoxAdapter(
                  child: _editing ? _buildEditForm(c) : _buildInfoCards(c, p, driver),
                ),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text('My Profile', style: TextStyle(color: c.text, fontSize: 22, fontWeight: FontWeight.w800)),
                  Text('Driver account', style: TextStyle(color: c.textSub, fontSize: 13)),
                ],
              ),
            ),
            const ThemeToggle(),
  ...
  Widget _buildAvatar(FleetColors c, UserProfile p, DriverProfile? driver) {
    ...
    final statusLabel = status == 'on_trip' ? 'On Trip' : status == 'off_duty' ? 'Off Duty' : 'Available';
    final statusColor = status == 'on_trip' ? AppColors.green : status == 'off_duty' ? AppColors.red : AppColors.blue;
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Text(name, style: TextStyle(color: c.text, fontSize: 20, fontWeight: FontWeight.w800)),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Assigned Truck', style: TextStyle(color: c.textSub, fontSize: 11)),
                  const SizedBox(height: 2),
                  Text(truck.plate, style: TextStyle(color: c.text, fontSize: 16, fontWeight: FontWeight.w700)),
                  if (truck.model != null)
                ...
                return Column(children: [
                  Padding(
                    ...
                    child: Row(children: [
                      Container(
                        width: 36, height: 36,
                        decoration: BoxDecoration(color: AppColors.orangeStart.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
